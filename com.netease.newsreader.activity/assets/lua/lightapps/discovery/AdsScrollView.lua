module("AdsScrollView", package.seeall)
...
function AdsScrollView.prototype:doInit()
   -- 模拟数据
   -- table.remove(self._activityList, #self._activityList)
   -- table.remove(self._activityList, #self._activityList)
   -- table.insert(self._activityList, {title="444"})
   -- table.insert(self._activityList, {title="555"})
   -- table.insert(self._activityList, {title="666"})
...
function AdsScrollView.prototype:createActivityScrollIndicators()
...
function AdsScrollView.prototype:updateActivityScrollIndicators()
   ...
   for i, imageView in ipairs(imageViews) do
      imageView:setWidth(indicatorWidth):setHeight(indicatorHeight):setTop(normalIndicatorTop):setBorderRadius(indicatorWidth/2):setBackgroundColor(Globals.getIndicatorColor())
      
   end

   imageViews[self._curActivityIndex]:setWidth(hilightIndicatorWidth):setHeight(hilightIndicatorHeight):setTop(hilightIndicatorTop):setBorderRadius(hilightIndicatorWidth/2):setBackgroundColor(Globals.getHilightedIndicatorColor())
end
