...
- (void)webViewDidFinishLoad:(UIWebView *)webView {
    ...
    if (_numRequestsLoading == 0 && ![[webView stringByEvaluatingJavaScriptFromString:@"typeof WebViewJavascriptBridge == 'object'"] isEqualToString:@"true"]) {
        NSString *filePath = [[NSBundle mainBundle] pathForResource:@"WebViewJavascriptBridge.js" ofType:@"txt"];
        NSString *js = [NSString stringWithContentsOfFile:filePath encoding:NSUTF8StringEncoding error:nil];
        NSString *rs=[webView stringByEvaluatingJavaScriptFromString:js];
    }
    
    if (_startupMessageQueue) {
        
        NSLog(@"webview webViewDidFinishLoad queuedMessage= %i",[_startupMessageQueue count]);
        for (id queuedMessage in _startupMessageQueue) {
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType {
    ...
    NSLog(@"webview shouldStartLoadWithRequest str=%@",str);
...
- (void)webViewDidStartLoad:(UIWebView *)webView {
    ...
    if(!activityIndicator){//
        activityIndicator= [[UIActivityIndicatorView alloc]
                            initWithFrame : CGRectMake((webView.frame.origin.x+webView.frame.size.width)/2.0f-50.0f, (webView.frame.origin.y+webView.frame.size.height)/2.0f-100.0f, 50.0f, 50.0f)] ;
        NSLog(@"webView.frame ===%@",NSStringFromCGRect(webView.frame));
        NSLog(@"activityIndicator ===%@",NSStringFromCGRect(CGRectMake((webView.frame.origin.x+webView.frame.size.width)/2.0f-50.0f, (webView.frame.origin.y+webView.frame.size.height)/2.0f-100.0f, 50.0f, 50.0f)));
